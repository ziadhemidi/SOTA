...
class TransBlock_UC(nn.Module):
    ...
    def __init__(self, in_channels=2, out_channels=2, nf=64, down_scale=2, img_size=256,
        ...
        self.encoder = nn.Sequential(
            nn.Conv2d(in_channels=in_channels, out_channels=nf, kernel_size=kernel1, stride=stride1, padding=1,
                      bias=True),
        ...
        self.decoder = nn.Sequential(
            nn.ConvTranspose2d(in_channels=nf, out_channels=nf, kernel_size=kernel2, stride=stride2, padding=1,
                               bias=True),
            nn.ReLU(inplace=True),
            nn.ConvTranspose2d(in_channels=nf, out_channels=out_channels, kernel_size=kernel1, stride=stride1,
                               padding=1, bias=True)
        )
    ...
    def forward(self, x, k0=None, mask=None):
        ...
        for i in range(self.num_iter):
            if i == 0:
                ...
                x = outputs[-1]
                x1, h1, b1_c1_att, b1_c2_att = self.block1(x, hidden=h1, h1_att=b1_c1_att, h2_att=b1_c2_att, k0=k0,
                                                           mask=mask)
                x2, h2, b2_c1_att, b2_c2_att = self.block2(x1, hidden=h2, h1_att=b2_c1_att, h2_att=b2_c2_att, k0=k0,
                                                           mask=mask)
                x3, h3, b3_c1_att, b3_c2_att = self.block3(x2, hidden=h3, h1_att=b3_c1_att, h2_att=b3_c2_att, k0=k0,
                                                           mask=mask)
